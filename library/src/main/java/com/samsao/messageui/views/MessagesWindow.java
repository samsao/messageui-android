...
public class MessagesWindow extends RelativeLayout {
        ...
        mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (pastVisibleItems == 0) {
                    if (mOnAskToLoadMoreCallback != null && !mIsLoading) {
                        mIsLoading = true;
                        mOnAskToLoadMoreCallback.onAskToLoadMore();
                    }
                } else {
                    mIsLoading = false;
                    mMessageAdapter.setMoreToLoad(mOnAskToLoadMoreCallback != null);
                }
            }
        });
    }
    ...
    public void showMessage(Message message) {
        ...
        if (mLinearLayoutManager.findFirstVisibleItemPosition() + mLinearLayoutManager.getChildCount() == mMessageAdapter.getItemCount() - (isScrollable() ? 1 : 0)
    ...
    /**
     * removes the callback for when the user reaches the top of the messages list and the progress indicator will not show anymore
     *
     */
    public void removeOnAskToLoadMoreCallback() {
        mOnAskToLoadMoreCallback = null;
        post(new Runnable() {
            @Override
            public void run() {
                mMessageAdapter.notifyDataSetChanged();
